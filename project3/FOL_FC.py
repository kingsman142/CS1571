...
def create_statements(logic): # Write out the actual proof to the user
    ...
    while len(all_facts) > 0 and len(rules) > 0 and not found_goal:
        ...
        while i < len(all_rules): # Loop through all the rules we've ever generated
            ...
            new_facts_and_rules = generate_all_facts_and_rules(all_rules[i], all_facts, 0, all_facts + new_facts) # Given this rule, loop over all the facts in this iteration and generate all possible new rules and facts
